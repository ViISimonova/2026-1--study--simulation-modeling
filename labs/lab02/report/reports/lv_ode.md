...
if length(spectrum_prey) > 0
    ...
    println(" Доминирующая частота колебаний жертв: ", round(dominant_freq_prey, digits=4), " Гц")
...
function analyze_parameter_sensitivity(param_index, values, param_name)
    println(" Анализ чувствительности к параметру: ", param_name)
    results = []
    for val in values
        p_test = copy(p_lv)
...
if false # Установите в true для выполнения анализа
    println(" 1. Влияние скорости размножения жертв (α):")
    analyze_parameter_sensitivity(1, [0.05, 0.1, 0.2, 0.3], "α")
    println(" 2. Влияние смертности хищников (γ):")
